...
- shared 模式下 `close_page` 只能关闭 Kcy-CLI 自己创建的 tab；无法证明是 Kcy-CLI 创建的 tab 会被策略层拒绝
...
- 内置工具 `load_skill(name)`：LLM 在 system prompt 看到匹配 description 时主动调用，Kcy-CLI 把 SKILL.md 正文（5KB 截断）写入 `SkillContextBuffer`，下一轮 user message 自动前置注入
...
设计意图：从「写工具」演进到「打包专家手册」。当工具堆成山（Kcy-CLI 当前内置 9 个 + MCP 60+ 工具），用 Skill 给 LLM 一份按场景展开的"专家手册"，比往 system prompt 里塞更多规则更可扩展。
...
| 形态 | 启用方式 | 视觉风格 |
|---|---|---|
| **inline 流式 TUI**（默认） | 直接运行 / `PAICLI_RENDERER=inline` | Claude Code / Qoder 风格：K 主题彩色开屏、主屏直出、transcript 当前位置的 `* ` 输入提示、JLine `Status` 托管的底部 dock（YOLO/HITL、MCP、Skill、model、phase、ctx、in/out/cache、cost、elapsed、cwd 等关键字段带克制彩色高亮；ctx 是当前上下文估算，in/out/cache 是最近调用统计）、右侧输入提示、行内可折叠工具块（`Read 3 files (ctrl+o to expand)`）、行内 git diff、HITL 单字符 `[y/n/a/s/m]` 提示 |
...
- 默认交互启动优先使用真实系统终端能力，并禁用 JLine `ffm` provider、启用 `jni` / `exec` provider；Windows 启动脚本会传入 `--enable-native-access=ALL-UNNAMED`，增强终端创建失败时仍会回退 dumb。
...
**为什么不叫沙箱**：本地 Agent CLI（参考 Claude Code / Cursor / Aider）默认都不做容器/VM 沙箱——沙箱削弱 Agent 能力、给虚假安全感、体验更差。生产级 Agent 沙箱实际是 microVM-level（Devin / Modal / Anthropic Computer Use 用 Firecracker / gVisor）。Kcy-CLI 的安全模型是 **HITL + 路径校验 + 命令快速拒绝 + 审计**，不是隔离。
...
MCP 子系统默认开启。`~/.paicli/mcp.json` 不存在时，Kcy-CLI 会自动创建默认 chrome-devtools 配置：
...
通常不需要用户预先切换；Agent 如果遇到登录页会自己调用 `browser_connect`。手工调试时也可以在 Kcy-CLI 内执行：
...
同一轮模型返回多个工具调用时，Kcy-CLI 会并行执行这些工具；如果工具之间有依赖关系，模型应分多轮调用。
