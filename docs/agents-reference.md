...
格式兼容 Claude Code：`command` + `args` = stdio，`url` + `headers` = Streamable HTTP。内置变量：`${PROJECT_DIR}`、`${HOME}`；其他 `${VAR}` 依次从系统环境变量、系统属性、项目 `.env`、用户 `~/.env` 读取。

检测到 `STEP_API_KEY` 时自动内置 `step_search` 远程 MCP（显式同名配置优先），用于 Step 3.7 Flash 的 `web_search` / `web_fetch` 优先代理。

自动创建的默认 chrome-devtools 配置在 Windows 使用 `npx.cmd`，在 macOS/Linux 使用 `npx`。
...
- shared 模式 close_page 只允许关闭 Kcy-CLI 创建的 tab
...
- 默认 CLI 交互路径优先使用真实系统终端能力，禁用 JLine `ffm` provider，并启用 `jni` / `exec` provider；Windows 启动脚本传入 `--enable-native-access=ALL-UNNAMED`。增强终端创建失败时回退 dumb，避免缺类或 native access 问题导致启动崩溃。
- `BottomStatusBar` 使用 JLine `Status` 托管底部 dock：状态行更新不再通过手写换行、上移或 `CLEAR_TO_EOS` 重绘；输入期把 LineReader 光标定位到 dock 上方输入行。dock 上层展示 YOLO/HITL 与 MCP/Skill 摘要，下层展示 Auto Model、model、phase、ctx、in/out/cache、cost、elapsed 与 cwd。关键字段使用 JLine `AttributedString` 做克制彩色高亮，同时保持纯文本格式和列宽裁剪稳定。`ctx` 只表示下一轮请求仍会携带的上下文估算，`in/out/cache` 表示最近任务的调用统计。
...
- `/compact` 使用完整摘要路径压缩当前 ReAct conversationHistory，期间显示动态 activity 面板，成功后刷新底部 ctx；不会清空长期记忆或待注入 SkillContextBuffer。
- `/export` 将当前 ReAct conversationHistory 导出到 `~/.paicli/exports/session-*.md`，包含完整 system prompt；命令不接受路径参数。
- 普通任务和斜杠命令提交后都会以 `>` 暗色整行块回写本轮提交文本（敏感片段会先脱敏），避免 JLine accept 后清除编辑行而看不到刚执行的输入。
