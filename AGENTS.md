...
- 开屏 Banner 使用无右边框的简洁布局，避免 CJK/ANSI 字宽导致右侧竖线错位；Phase 22 后默认是 K 主题彩色 logo + Qoder 风格首屏，只展示模型、MCP、Skill、ReAct 状态和三条 getting-started tips，不再把 MCP server 明细刷成启动日志。
...
- 默认交互终端应优先使用真实系统终端能力，不要直接强制 `dumb(true)`；默认禁用 JLine `ffm` provider、启用 `jni`/`exec` provider，Windows 启动脚本通过 `--enable-native-access=ALL-UNNAMED` 允许 JLine native provider；增强终端创建失败时再回退 dumb，避免缺类或 native access 问题直接导致启动崩溃。
